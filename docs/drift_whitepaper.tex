...
\begin{abstract}
DRIFT is a constraint-first dashboard for monitoring polar motion as a geometric dynamical system over the observed record. It emphasizes the strongest findings of the underlying paper---low-dimensional confinement, near-planar organization, and robust two-state structure---while treating directional alignment and geomagnetic comparison more cautiously. The system combines automated data retrieval, cache-aware preprocessing, rolling diagnostics, exploratory transition forecasting, and interactive visualization in a single deployable application. This white paper describes the dashboard's guiding principles, its frame of reference, the ingestion and caching pipeline, and the interpretation of the major panels presented to end users.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Cross-panel coherence.} The 2D charts, forecast layer, and 3D view should all describe the same underlying state without conflicting conventions.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item short-horizon transition forecasts as exploratory summaries.
\end{itemize}
...
Derived rolling diagnostics & Local Python computation & On rebuild / on demand & Drift axis, angular relations, conditional lag response, and forecast inputs \\
\bottomrule
\end{longtable}
...
The system tracks geomagnetic dipole orientation as a vector time series. Where a fully physical dipole-moment source is unavailable, the dashboard can fall back to a derived axis and a strength proxy from geomagnetic activity. The dashboard explicitly labels the provenance of the displayed dipole series so the user can distinguish between a direct series and a derived fallback.
...
The angle diagnostics panel compares relative angular measures such as out-of-plane tilt and in-plane alignment. These quantities are useful for spotting sustained drifts, abrupt bends, repeated crossings, or changes in coherence between the drift vector and the principal frame.

\subsection{Transition Forecasting}
Rather than reducing the forecast to a single probability, DRIFT computes a time-resolved forecast curve. This allows the user to inspect:
\begin{itemize}[leftmargin=1.5em]
  \item the most likely transition horizon,
  \item the expected time to transition,
  \item the cumulative likelihood within a chosen horizon,
  \item the active state conditioning used by the model.
\end{itemize}

This early-warning framing is especially useful when probability remains modest but the expected time is contracting. The forecast should be interpreted as a lag-conditioned summary of how the present state compares with earlier transition-like episodes, not as a deterministic event prediction.
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Transition Forecast}
...
\subsection{Transition Forecast}
This panel reports the forward probability curve for a state transition. The key summary cards are:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Peak Time:} the horizon of highest forecast density,
  \item \textbf{Expected Time:} the mean forecast delay, now also shown as a projected calendar date,
  \item \textbf{$P(\leq 30d)$ or equivalent windowed probability:} near-term cumulative risk,
  \item \textbf{Alert Level:} a compact severity cue.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item the geomagnetic dipole,
  \item recent vector paths with angular-velocity-aware coloring.
\end{itemize}

Small angular separations suggest closer directional agreement. Rapidly recolored path segments indicate faster reorientation and therefore deserve closer comparison with the chart-based diagnostics. Visual proximity alone should not be taken as proof of coupling.
...
\subsection{Coupling and Auxiliary Panels}
Additional panels such as coupling plots, phase portraits, and orthogonal deviation are meant to provide context and cross-validation. A meaningful interpretation typically emerges when multiple panels tell the same story: for example, geometric confinement, coherent loop structure, drift-axis migration, and then only secondarily any matching geomagnetic change.
...
The application is packaged as a Docker image and deployed to a Kamatera virtual machine. Nginx handles default web traffic on ports 80 and 443, proxies to the container on port 3000, and uses Let's Encrypt certificates for HTTPS. This deployment model has three advantages:
\begin{enumerate}[leftmargin=1.5em]
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item use the 3D and geomagnetic panels for contextual comparison,
  \item read the Transition Forecast as an exploratory summary of transition-like risk in the current state.
\end{enumerate}

\section{Current Limitations}
Several limits are important:
\begin{itemize}[leftmargin=1.5em]
  \item some geomagnetic strength reporting remains proxy-based rather than a full physical dipole-moment estimate,
  \item forecast outputs depend on the quality and stability of the rolling diagnostic layer,
  \item directional alignment and apparent axis stability are weaker claims than the geometric structure itself,
  \item geomagnetic comparisons provide context rather than standalone evidence of causation,
